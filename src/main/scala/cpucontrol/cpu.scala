...
class ControlUnit extends Module {
  val io = IO(new Bundle {
    ...
    val insns = Input(Vec(1024, UInt(32.W)))
  ...
  when(counterReg ===1.U){
    io.nPC := io.PC + 4.U
    io.insn := io.insns((io.PC >> 2.U).asUInt)
  }.otherwise{
    io.nPC := io.PC 
    io.insn := "h0000_0013".U(32.W)
  }
...
class RISCVCPUv2 extends Module {
  ...
  val DMemory = RegInit(VecInit(Seq.fill(1024)(0.U(32.W))))
  val IMemory = RegInit(VecInit(Seq.fill(1024)(NOP)))
...
class RISCVCPUv2Bubble extends Module {
  val io = IO(new Bundle {
    val rvfi = new RVFI_IO
  })
  val LD = "b000_0011".U(7.W)
  ...
  val cont = Module(new ControlUnit).io 
  cont.PC := PC
  cont.insns := IMemory
  cont.reset := io.rvfi.rst
...
class self extends Module{
  ...
  val cpu2 = Module(new RISCVCPUv2Bubble)
  for( i <- 0 until 8){
    if(i==0||i==1){
  ...
  // cpu1.io.rvfi.insn_in := io.insn_in
  // cpu2.io.rvfi.insn_in := io.insn_in
...
object x extends App {
  new ChiselStage().emitSystemVerilog(new self())
}