...
class cpuSpec5 extends AnyFlatSpec with ChiselScalatestTester {
  ...
  "mfcpu ste assert complicate" should "pass" in {
    test(new RISCVCPUv2error()) { dut =>
      ...
      def cons(i:Int) = {
        at_cycle(
          Imply((!(ldrd_0 == ldrd_1))&(!(mem_data_in_0 == mem_data_in_1))&constrain(), is_bv(dut.io.rvfi.pc_wdata, 16)) &&
          Imply((!(ldrd_0 == ldrd_1))&((mem_data_in_0 == mem_data_in_1))&constrain(), is_bv(dut.io.rvfi.pc_wdata, 8+pcj)) 
          ,
          i
        )
      }
      ...
      val assert = new Assert(ant_, cons_, () => new RISCVCPUv2error(), SMT)
      assert.STE
