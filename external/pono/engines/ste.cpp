...
bool Ste::step(int i)
{
  ...
  std::cout<<"     push ant :"+antecedent[i]->to_string()<<std::endl;
  ...
  /**
     * czh
   */
  std::cout<<"     push cons :"+consequent[i]->to_string()<<std::endl;
  Term con_i = unroller_.at_time(consequent[i],i);
  std::cout<<"     push cons_i :"+con_i->to_string()<<std::endl;
  //  Term con_i = unroller_.at_time(con(i,x,solver_),i);
...
ProverResult Ste::check_until_backward(int k)
{
  initialize();
...
bool Ste::step_backward(int i)
{
  ...
  Term ant_i = unroller_.at_time(antecedent[i],i);
  //  Term con_i = unroller_.at_time(con(i,x,solver_),i);
  std::cout<<"     push ant_i :"+ant_i->to_string()<<std::endl;
  logger.log(3,"     push ant:"+ant_i->to_string());
  solver_->assert_formula(ant_i);
  ...
  Term con_i = unroller_.at_time(consequent[i],i);
  std::cout<<"     push cons :"+con_i->to_string()<<std::endl;
  Term not_con_i = solver_->make_term(Not,con_i);
  solver_->assert_formula(not_con_i);
  reached_k_ = i;
